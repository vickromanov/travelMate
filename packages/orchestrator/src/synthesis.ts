...
export async function synthesizePlan(
  ...
  // Strip physically impossible blocks that the LLM sometimes hallucinates:
  // arrival-day dining before 14:00 and placeholder-labeled blocks on any day.
  stripImpossibleBlocks(plan);
    ...
    if (parsed.success) {
      ...
      day.blocks = day.blocks.filter((b) => {
        if (PLACEHOLDER_LABEL_RE.test(b.label ?? "")) {
          console.warn(`[synthesis] day ${sd.dayNumber}: stripped placeholder block "${b.label}"`);
          return false;
        }
        if (sd.dayNumber === 1 && b.category === "DINING") {
          const hhmm = b.scheduledTime.replace(":", "");
          if (parseInt(hhmm, 10) < 1400) {
            console.warn(`[synthesis] day 1: stripped pre-arrival DINING "${b.label}" at ${b.scheduledTime}`);
            return false;
          }
        }
        return true;
...
function stripImpossibleBlocks(plan: TripPlan): void {
  const PLACEHOLDER_LABEL_RE = /placeholder for day \d+/i;
  for (const day of plan.days) {
    day.blocks = day.blocks.filter((b) => {
      if (PLACEHOLDER_LABEL_RE.test(b.label ?? "")) {
      ...
      if (day.dayNumber === 1 && b.category === "DINING") {
        const hhmm = b.scheduledTime.replace(":", "");
        if (parseInt(hhmm, 10) < 1400) {
          return false;
        }
